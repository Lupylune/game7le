...
import { CROISES5, CROISES5_RARE } from '../data/lexique';
 ...
 * Pool : lemmes de Lexique 3.83 ayant une définition Wiktionnaire — mots
 * courants pour le quotidien, mots plus rares pour le défi difficile.
 */

const poolDe = (src: string[]) => src.map((p) => p.split(':')[0]).filter((w) => DEFS5.has(w));
const POOL = poolDe(CROISES5);
const POOL_RARE = poolDe(CROISES5_RARE);
...
function tryFill(rng: RNG, pool: string[], R: number[], C: number[]): Croise | null {
...
export function genCroise(rng: RNG, difficile = false): Croise | null {
  const pool = difficile ? POOL_RARE : POOL;
  for (let attempt = 0; attempt < 25; attempt++) {
    const R = pick(rng, SOUS_ENSEMBLES);
    const C = pick(rng, SOUS_ENSEMBLES);
    const grille = tryFill(rng, pool, R, C);
